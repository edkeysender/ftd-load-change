...
echo " 2. Install it to always start ELEVATED (needed for clock/ssh/CPU-temp guards) -"
echo "    the agent does this itself:   .\\simagent.exe -install"
echo "    (accepts one UAC prompt, registers a logon task, runs elevated ever after."
echo "     deploy/install-agent.ps1 is the scriptable equivalent. Quick test: .\\simagent.exe)"
